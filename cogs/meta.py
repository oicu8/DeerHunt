...
class Meta(commands.Cog):
    ...
    async def uptime(self, ctx):
        ...
        await self.bot.hint(ctx=ctx,
                            message=_("The bot frequently reboots for updates. Don't worry, it has a 99.8% of uptime.",
                                      language))
    ...
    async def wiki(self, ctx):
        await self.bot.send_message(ctx=ctx, message="https://deerhunt.me/")
    ...
    async def help(self, ctx):
        ...
        await self.bot.send_message(ctx=ctx, message=_(
            "Here is the command list: http://deerhunt.me/command-list/ | If you need anything, feel free to join the "
    ...
    async def time(self, ctx):
        ...
        await self.bot.send_message(ctx=ctx, message=_(
            "It's `{hour}:{minutes}:{seconds}` in the GMT timezone (also known as UTC±00:00), which is the one used by the bot. \nYou can use this website to compare it to yours: https://time.is/compare/GMT",
            language).format(hour=hour, minutes=minutes, seconds=seconds))
    ...
    async def freetime(self, ctx):
        ...
        await self.bot.send_message(ctx=ctx, message=_(
            ":alarm_clock: Next giveback of weapons and magazines in {sec} seconds ({hours} hour(s) and {minutes} minute(s)).",
            language).format(sec=seconds_left, hours=hours, minutes=minutes))
        await self.bot.hint(ctx=ctx, message=_(
            "A giveback tops up your magazines and gives you your weapon back for free (if confiscated), but it doesn't affect the bullets in your current magazine. "
