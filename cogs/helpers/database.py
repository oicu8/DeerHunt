...
class Database:
    ...
    async def get_channel_dbid(self, channel):
        ...
        # For this, we can query deerhunt/channels with the right guild+channel id combo,
    ...
    async def top_scores(self, channel, sorted_by, second_sort_by):
        ...
        row = self.database.query(f"SELECT * FROM players WHERE channel_id=:channel_id AND (exp <> 0 OR killed_deer > 0) ORDER BY {sorted_by}, {second_sort_by}", channel_id=channel_id)
    ...
    async def get_stat(self, channel, user, stat: str):
        ...
        while not row:
            ...
            # Now that we have the ID, we can get into deerhunt/players and find the player we need
    ...
    async def set_stat(self, channel, user, stat: str, value: int):
        ...
        # Now that we have the ID, we can get into deerhunt/players and update the player we need
        ...
        if cond:
            ...
            embed.set_footer(text='DeerHunt - Discord version by oicu8#3256', icon_url='https://i.imgur.com/18gMmyy.png')
            try:
