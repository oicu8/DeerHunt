...
class SuperAdmin(commands.Cog):
    ...
    async def webinterface_roles(self, ctx):
        roles = {
            ...
            '$partners'                      : 'Partner',
            '$donators'                      : 'Donator',
            '$enigma_event_winners_june_2018': 'DeerEnigma Event Winner 2018',
    ...
    async def server_owners_manager(self, ctx):
        ...
        active_guilds = {channel.guild for channel in self.bot.deer_planning.keys()}
    ...
    async def broadcast(self, ctx, *, bc: str):
        ...
        channels = list(self.bot.deer_planning.keys())
        total = len(channels)
        i = 0
        f = 0
