...
class Experience(commands.Cog):
    ...
    async def list(self, ctx):
        ...
        await self.bot.send_message(ctx=ctx, message=_("Here is the list of all the shops items : https://deerhunt.me/shop-items/ . Thanks ", language))
    ...
    async def item3(self, ctx):
        ...
        if await self.bot.db.get_stat(message.channel, message.author, "ap_ammo") < time.time():
            ...
            await self.bot.hint(ctx=ctx, message=_("For the next 24 hours, you will deal double damage to deer. This does **not** stack with explosive ammo", language))
    ...
    async def item4(self, ctx):
        ...
        if await self.bot.db.get_stat(message.channel, message.author, "explosive_ammo") < time.time():
            ...
            await self.bot.hint(ctx=ctx, message=_("For the next 24 hours, you will deal triple damage to deer. This does **not** stack with AP ammo and trumps it if you have any.", language))
    ...
    async def item6(self, ctx):
        ...
        if await self.bot.db.get_stat(message.channel, message.author, "graisse") < time.time():
            ...
            await self.bot.hint(ctx=ctx, message=_("This reduces jamming risks by 50 percent for a day, for only 8 exp points. It's really useful for new players. Use `dh!deertats` to check your "
    ...
    async def item9(self, ctx):
        ...
        if await self.bot.db.get_stat(message.channel, message.author, "silencieux") < time.time():
            ...
            await self.bot.hint(ctx=ctx, message=_("The silencer works best if every hunter uses one, or if you're alone. "
                                                   "With it, no deer will run away before you can kill them. What a good deal, they only cost 5 exp!", language))
    ...
    async def item10(self, ctx):
        ...
        if await self.bot.db.get_stat(message.channel, message.author, "trefle") < time.time():
            ...
            await self.bot.send_message(ctx=ctx, message=_(":money_with_wings: You bought a fresh 4-leaf clover, "
                                                           "which will give you {exp} more exp points for each killed deer for the next day.", language).format(**{"exp": exp}))
    ...
    async def item20(self, ctx):
        ...
        if await self.bot.db.get_pref(ctx.channel, "disable_decoys_when_deer_are_sleeping"):
            sdstart = await self.bot.db.get_pref(ctx.channel, "sleeping_deer_start")
            sdstop = await self.bot.db.get_pref(ctx.channel, "sleeping_deer_stop")
        ...
        if currently_sleeping:
            await self.bot.hint(ctx=ctx, message=_(":money_with_wings: Deer are resting right now, so your decoy probably didn't work...", language))
            return

        await self.bot.send_message(ctx=ctx,
                                    message=_(":money_with_wings: A deer will appear in the next 10 minutes on the channel, thanks to {mention}'s decoy. They bought it for 8 exp!", language).format(
                                        **{"mention": message.author.mention}))
        dans = random.randint(0, 600)

        async def spawn_decoy_deer():
            await asyncio.sleep(dans)
            await spawning.spawn_deer(self.bot, ctx.channel)

        asyncio.ensure_future(spawn_decoy_deer())
    ...
    async def item21(self, ctx):
        ...
        if random.randint(1, 6) == 1:
            self.bot.deer_planning[message.channel] += 1
            tempmsg = "The deer are sensing your delicious wafers..."
        ...
        await self.bot.send_message(ctx=ctx,
                                    message=_(":money_with_wings: You put some bread on the channel to attract deer. " + tempmsg, language))
    ...
    @shop.command(name="23", aliases=["fake_deer", "mechanical_deer", "mechanical", "fake"])
    @checks.have_exp(40)
    async def item23(self, ctx):
        """Buy a mechanical deer (40 exp)
        !shop 23"""
        message = ctx.message
        _ = self.bot._;
        language = await self.bot.db.get_pref(ctx.channel, "language")
        ...
        await self.bot.db.add_to_stat(message.channel, message.author, "exp", -40)
        await self.bot.send_message(ctx=ctx, force_pm=True, message=_(":money_with_wings: You prepared a mechanical deer on the channel for 40 exp. It's wrong, but so funny!", language))

        async def spawn_mech_deer():
            await asyncio.sleep(90)
            deer = await deer.MechanicalDeer.create(self.bot, ctx.channel, user=ctx.message.author)
            await spawning.spawn_deer(self.bot, ctx.channel, instance=deer, ignore_event=True)

        asyncio.ensure_future(spawn_mech_deer())
