...
class Scores(commands.Cog):
    ...
    async def top(self, ctx, *, args: str = ""):
        ...
        await self.bot.send_message(ctx=ctx, message=_("The scores are online at http://deerhunt.api-d.com/web/deertats.php?cid={channel_id}", language).format(
            channel_id=channel.id))
        # await self.bot.hint(ctx=ctx, message="The following will disappear in a few days. If you notice a bug, please report it on the DeerHunt server : <https://discord.gg/G4skWae>. Thanks! ")
            ...
            while reaction:
                if changed:
                    ...
                    if scores_to_process:
                        ...
                        embed.url = 'https://deerhunt.me/'  # TODO: get the webinterface url and add it here inplace
                        ...
                        for joueur in scores_to_process:
                            ...
                            if (not "killed_deer" in joueur) or (not joueur["killed_deer"]):
                                joueur["killed_deer"] = _("None!", language)
                        ...
                        try:
                            await message.edit(content="<:official_Deer_01:439546719177539584>", embed=embed)
...
"""
    ...
    @commands.command(aliases=["stats", "deer_stats"])
    @checks.is_channel_enabled()
    @commands.cooldown(2, 60, BucketType.user)
    @checks.had_giveback()
    async def deertats(self, ctx, target: discord.Member=None):
        ...
        await self.bot.send_message(ctx=ctx, message=_("Your deertats are waiting for you at http://deerhunt.api-d.com/web/deertats.php?cid={channel_id}&pid={player_id}", language).format(
        ...
        deertats_message = await self.bot.send_message(ctx=ctx, force_pm=await self.bot.db.get_pref(ctx.guild, "pm_stats"),
                                                        message=_("Generating DeerHunt statistics for you, please wait...", language), return_message=True)
        await deertats_message.add_reaction(first_page_emo)
        await deertats_message.add_reaction(prev_emo)
        await deertats_message.add_reaction(next_emo)
        ...
        while reaction:
            if changed:
                ...
                embed.url = 'https://deerhunt.me/'
                embed.colour = discord.Colour.green()
                embed.set_footer(text='DeerHunt', icon_url='https://i.imgur.com/18gMmyy.png')
                ...
                if current_page == 1:
                    ...
                    if await gs("killed_deer") > 0:
                        ratio = round(await gs("exp") / await gs("killed_deer"), 4)
                    else:
                        ratio = _("No deer killed", language)

                    embed.add_field(name=_("Deer killed", language), value=str(await gs("killed_deer")))
                    embed.add_field(name=_("Super deer killed", language), value=str(await gs("killed_super_deer")))
                    ...
                    embed.add_field(name=_("Ratio (exp/deer killed)", language), value=str(ratio))
                    ...
                    embed.add_field(name=_("Shots without deer", language), value=str(await gs("shoots_no_deer")))
                    embed.add_field(name=_("Shots that frightened a deer", language), value=str(await gs("shoots_frightened")))
                    embed.add_field(name=_("Shots that harmed a deer", language), value=str(await gs("shoots_harmed_deer")))
                ...
                try:
                    #ctx.logger.debug("Deertats : " + str(embed.to_dict()))
                    await deertats_message.edit(content="<:official_Deer_01:439546719177539584>", embed=embed)
            ...
            def check(reaction, user):
                cond = reaction.message.id == deertats_message.id
                cond = cond and user == ctx.message.author
            ...
            if res:
                ...
                if emoji == next_emo:
                    ...
                    if current_page == total_page:
                        ...
                        current_page += 1
                    try:
                        await deertats_message.remove_reaction(emoji, user)
                    except discord.errors.Forbidden:
                        #ctx.logger.exception("Forbidden from deertats 1") #!deertats 345315631236907008
                    ...
                    try:
                        ...
                        #ctx.logger.exception("Forbidden from deertats 2")
                        pass  # await self.bot.send_message(message.channel, _("I don't have the `manage_messages` permissions, I can't remove reactions. Warn an admin for me, please ;)", language))
                    ...
                    try:
                        ...
                        #ctx.logger.exception("Forbidden from deertats 3")
                        pass  # await self.bot.send_message(message.channel, _("I don't have the `manage_messages` permissions, I can't remove reactions. Warn an admin for me, please ;)", language))
            else:
                reaction = False
                try:
