...
$dbname   = "DeerHunt";
$username   = "deerhunt_web";
$password   = "deerhunt_web";

$admins = array(
    365530555233599499, // Oicu8#3256
);
$bug_hunters = array(
...
if (isset($_GET['cid'])) {
    ...
    if (isset($_GET['pid'])) { // On a la channel et le player
        ...
        $player_stats['achievements'] = array(
            ...
            "baby_lover"     => $player_stats['killed_baby_deer'] < 5,
            "murderer"       => $player_stats['murders'] > 0,
            "first_blood"    => $player_stats['killed_deer'] > 1,
            "deer_killed_1" => $player_stats['killed_deer'] > 10,
            "deer_killed_2" => $player_stats['killed_deer'] > 100,
            "deer_killed_3" => $player_stats['killed_deer'] > 500,
            "deer_killed_4" => $player_stats['killed_deer'] > 1000,
            "deer_killed_5" => $player_stats['killed_deer'] > 2000,
            "deer_killed_6" => $player_stats['killed_deer'] > 4000,
        ...
        if (isset($_GET['api-agent'])){
            ...
            echo $twig->render('deertats_player.twig', $player_stats);
            die();
        }
        ...
        $players_array_query = $conn->prepare("SELECT * FROM players WHERE channel_id=:channel_id AND (exp <> 0 OR killed_deer > 0) ORDER BY exp DESC");
        ...
        $total_baby_deer       = 0;
        $total_moad_deer       = 0;
        $total_mechanical_deer = 0;
        $total_players_killed   = 0;


        foreach ($players_array as &$player) {
            $total_normal_deer     += $player['killed_normal_deer'];
            $total_super_deer      += $player['killed_super_deer'];
            $total_baby_deer       += $player['killed_baby_deer'];
            $total_moad_deer       += $player['killed_mother_of_all_deer'];
            $total_mechanical_deer += $player['killed_mechanical_deer'];
        ...
        $passed = [
            ...
            "total_normal_deer"     => $total_normal_deer,
            "total_super_deer"      => $total_super_deer,
            "total_baby_deer"       => $total_baby_deer,
            "total_moad_deer"       => $total_moad_deer,
            "total_mechanical_deer" => $total_mechanical_deer,
        ...
        if (isset($_GET['api-agent'])){
            ...
            echo $twig->render('deertats_channel.twig', $passed);
            die();
        }
