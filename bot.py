...
# Installation tutorial : https://kb.api-d.com/en/code/deerhunt-how-to/
...
async def get_prefix(bot, message):
    extras = [await bot.db.get_pref(message.channel, "prefix"), "deerhunt", "dh!", "dh", "Dh", "Dh!", "dH!", "dH", "DH!", "DH"]
...
class DeerHunt(commands.AutoShardedBot):
    ...
    async def on_ready(self):
        ...
        await self.log(level=30, title="Bot is ready", message=f"The bot has restarted. We can now see {len(self.guilds)} guilds on {self.shard_count} shards, and deer can now spawn", where=None)
    ...
    async def on_guild_remove(self, guild):
        ...
        for channel in guild.channels:
            self.deer_planning.pop(channel, None)

        logger.info(f"Before removing deer, we had {len(bot.deer_spawned)} deer")

        self.deer_spawned = [deer for deer in self.deer_spawned if deer.channel not in guild.channels]

        logger.info(f"After removing deer, we have {len(bot.deer_spawned)} deer")

    async def on_guild_channel_delete(self, channel):
        logger.info(f"Channel removed (from discord) : {channel} :(!")

        self.deer_planning.pop(channel, None)
        self.deer_spawned = [deer for deer in self.deer_spawned if deer.channel != channel]
    ...
    async def on_command_error(self, context, exception):
        ...
        if isinstance(exception, discord.ext.commands.errors.CommandNotFound):
            ...
            await self.hint(ctx=context, message=_("This command is reserved for the bot owners. "
                                                   "If you think this is an error, please contact my owner at the DeerHunt Support Server (see `dh!help`).", language))
            return
        elif isinstance(exception, checks.NoVotesOnDBL):
            await self.send_message(ctx=context, message=_(":x: You haven't voted for DeerHunt on DiscordBotList for a while.", language))
            await self.hint(ctx=context, message=_("Support the bot to use this command by voting at <https://discordbots.org/bot/deerhunt>. "
                                                   "Be aware that the votes can take a minute to be registered by Deerhunt"
                                                   "If you think this is an error, please contact my owner at the DeerHunt Support Server (see `dh!help`).", language))
            return
...
bot = DeerHunt(command_prefix=get_prefix, case_insensitive=True)
...
try:
    ...
    bot.loop.run_until_complete(spawning.make_all_deer_leave(bot))
